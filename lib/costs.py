...
class CostModelCollisionSphere():
    ...
    def calc(self, x, u):
        ...
        if self.dist < self.d_thres: # d_thresh = radius + margin 
            self.obs_status = True #near to the obstacle
            self.L = 0.5*self.w_obs*(self.dist-self.d_thres)**2 # cost function 
        else:
            self.obs_status = False
            self.L = 0
        return self.L
    
    def calcDiff(self, x, u, recalc = False): # calc  derivative of costs 
        if recalc:
            self.calc(x, u)
        self.J   = self.sys.compute_Jacobian(x, self.ee_id)[:3]  #Only use the translation part
        p,_      = self.sys.compute_ee(x, self.ee_id)
        ...
        if self.obs_status:
            ...
            self.Lx = np.zeros(self.Dx)
            self.Lx[:3]  = self.w_obs*Jtemp.dot(self.dist-self.d_thres)
            self.Lxx = np.zeros((self.Dx, self.Dx))
            self.Lxx[:3, :3] = self.w_obs*Jtemp.T.dot(Jtemp)
...
class CostModelCollisionGeneral():
    '''
    The collision cost model between the end-effector and a general pybullet obstacle
    '''
    def __init__(self, sys, r_id, o_id, ee_id=0, w_obs = 1.,  max_dist = 10000):
        self.sys = sys
        self.Dx, self.Du = sys.Dx, sys.Du
        
        self.r_id = r_id #robot id
        self.o_id = o_id #obstacle id
    ...
    def calc(self, x, u):    
        p.resetBasePositionAndOrientation(self.r_id, x[:3], (0,0,0,1))
        try:
            col_info = p.getClosestPoints(self.r_id, self.o_id, self.max_dist)[0]
        except:
            self.obs_status = False
            self.L = 0
            return self.L
        ...
        self.pr, self.po, self.dist = np.array(col_info[5]), np.array(col_info[6]), np.array(col_info[8])
        
        self.res = self.pr - self.po        
        if self.dist < 0: 
            self.obs_status = True #near to the obstacle
            self.L = 0.5*self.w_obs*(self.dist)**2 # cost function 
        else:
            self.obs_status = False
            self.L = 0
    ...
    def calcDiff(self, x, u, recalc = False): # calc  derivative of costs 
        ...
        if self.obs_status:
            Jtemp = (self.res)/self.dist
            self.Lx = np.zeros(self.Dx)
            self.Lx[:3]  = self.w_obs*Jtemp.dot(self.dist)
            self.Lxx = np.zeros((self.Dx, self.Dx))
            self.Lxx[:3, :3] = self.w_obs*Jtemp.T.dot(Jtemp)
        else: # if no collision 
            self.Lx = np.zeros(self.Dx)
